...
class Cinema_model extends CI_Model
{
	...
	public function get_all($where = array(), $order = "cinema.id ASC")
	{
	...
	{
		$this->db->where(["cinema.id!=" => ""]);

		if (!empty($this->input->post('search'))) {
			$this->db->group_start();
			$this->db->like('cinema.title', $this->input->post('search'), 'both');
			$this->db->group_end();
		}
		...
		foreach ($this->column_search as $item) {
			...
			if (!empty($postData['search']) && ($item == "cinema.title")) {
				// first loop
				if ($i === 0) {
					// open bracket
					$this->db->group_start();
