    ...
    <div class="container-fluid mt-3">
        ...
        <div class="row">
            ...
            <?php if (!empty($news && $news_categories)) : ?>
                <?php foreach ($news_categories as $k => $v) : ?>
                    <?php
                        @$haberArray = explode(" ", $v->title);
                        $first = @$haberArray[0];
                        $last = @ltrim($v->title, $first . ' ');
                    ?>
                    <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 mt-3">
                        <h4 class="title"><b><?= $first ?></b> <?= $last ?></h4>
                        <div class="owl-wrapper">
                            <div class="owl-carousel owl-trends owl-theme mt-3">
                                    ...
                                    <?php if ($value->category_id == $v->id) : ?>
                                        <a href="<?= base_url("haber/{$value->seo_url}"); ?>">
                                            <div class="card item mb-3 dark shadow-none">
                                                <div class="card-body p-0 m-0">
                                                    <div class="position-relative">
                                                        <img src="<?= get_picture("news_v", $value->img_url) ?>" class="img-fluid">
                                                    ...
                                                    <div>
                                                        <div class="d-inline-block mr-1 py-2 px-2">
                                                            ...
                                                            <?php foreach ($writers as $writer_key => $writer_value) : ?>
                                                                <?php if ($value->writer_id == $writer_value->id) : ?>
                                                                    <?= $writer_value->full_name ?>
                                                                <?php endif; ?>
                                                            <?php endforeach; ?>
                                                        </div>
                                                        <div class="d-inline-block">
                                                            <i class="fa fa-clock-o mr-1"></i><?= turkishDate("d F Y, l H:i", $value->createdAt);  ?>
                                                        </div>
                                                    </div>
                                                    <h6 class="px-1 py-3"><b><?= $value->title; ?></b></h6>
                                                </div>
                                            </div>
                                        </a>
                                    <?php endif ?>
